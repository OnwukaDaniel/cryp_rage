...
final getBotCryptoPriceEntryRepo = Provider((_) => GetBotCryptoPriceEntryRepo());

class GetBotCryptoPriceEntryRepo {
  Future<ApiResponse> getBotCrypto(String pair) async {
    final headers = {
      'Content-Type': 'application/json; charset=UTF-8',
    };

    Response response = await http.get(
      Uri.parse('https://api.coinbase.com/v2/exchange-rates?currency=$pair'),
      headers: headers,
    );
